  -- Code to collect all items ordered by 1 restaurant. Returns the items with the biggest price change
  -- Will be used to hehlp show users how suppliers are screwing them over.
  -- Also provides better arggument as to why bother using the Foodweb app.
  ...
  reverse_rank,
      ...
      WHERE i.restaurant_id = 12 -- Change this to select a different restaurant
  ),
  price_changes AS (
      SELECT
